...
def eval_linear_log(masked, truth):
    ...
    for ix, row in masked.iterrows():
        ...
        if len(v_s) >= 2:
            f = interp1d(np.log(np.array(v_s)/S), v_iv, kind='linear', fill_value='extrapolate')
            for j, c in enumerate(ce_cols):
                if pd.isna(row[c]): filled.at[ix, c] = float(f(np.log(strikes_ce[j]/S)))
        elif len(v_s) == 1:
            for c in ce_cols:
        ...
        if len(v_s) >= 2:
            ...
            for j, c in enumerate(pe_cols):
                if pd.isna(row[c]): filled.at[ix, c] = float(f(np.log(strikes_pe[j]/S)))
        elif len(v_s) == 1:
            for c in pe_cols:
                if pd.isna(row[c]): filled.at[ix, c] = v_iv[0]
...
print("Linear Log-Moneyness MSE:", eval_linear_log(masked, truth))
