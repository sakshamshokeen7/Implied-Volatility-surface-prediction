"""
Test IterativeImputer with Tree Models
"""
import pandas as pd
import numpy as np
...
def validate_model(estimator, name):
    scores = []
    n_trials = 2
    for trial in range(n_trials):
        ...
        imp = IterativeImputer(estimator=estimator, max_iter=10, random_state=42)
...
print("Evaluating Tree Estimators inside IterativeImputer...")
validate_model(ExtraTreesRegressor(n_estimators=30, random_state=42, n_jobs=-1), "ExtraTrees")
validate_model(RandomForestRegressor(n_estimators=30, random_state=42, n_jobs=-1), "RandomForest")
