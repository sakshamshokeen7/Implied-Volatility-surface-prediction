...
def eval_smooth(masked, truth, s):
    ...
    for ix, row in masked.iterrows():
        ...
        if len(v_s) > 3:
            spline = UnivariateSpline(v_s, v_iv, s=s, ext=0)
            for j, c in enumerate(ce_cols):
                if pd.isna(row[c]): filled.at[ix, c] = spline(strikes_ce[j])
        ...
        if len(v_s) > 3:
            ...
            for j, c in enumerate(pe_cols):
                if pd.isna(row[c]): filled.at[ix, c] = spline(strikes_pe[j])
...
print("Smoothing Spline s=0.001 MSE:", eval_smooth(masked, truth, 0.001))
print("Smoothing Spline s=0.005 MSE:", eval_smooth(masked, truth, 0.005))
print("Smoothing Spline s=0.010 MSE:", eval_smooth(masked, truth, 0.010))
