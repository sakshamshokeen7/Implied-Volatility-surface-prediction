...
def eval_interp(masked, truth, kind='linear'):
    ...
    for ix, row in masked.iterrows():
        ...
        if len(v_s) >= 4:
            f = interp1d(v_s, v_iv, kind=kind, fill_value='extrapolate')
    ...
    for ix, c, tv in truth:
        pv = filled.at[ix, c]
        if pd.notna(pv): y_t.append(tv); y_p.append(np.clip(pv, 0.001, 6.0))
    return mean_squared_error(y_t, y_p)

print("Linear MSE:", eval_interp(masked, truth, 'linear'))
print("Quadratic MSE:", eval_interp(masked, truth, 'quadratic'))
print("Cubic MSE:", eval_interp(masked, truth, 'cubic'))
