...
def eval_mixed_interp(masked, truth, interior_kind='cubic'):
    filled = masked.copy()
    for ix, row in masked.iterrows():
        ...
        if len(v_s) >= 4:
            f_in = interp1d(v_s, v_iv, kind=interior_kind)
            f_ex = interp1d(v_s, v_iv, kind='linear', fill_value='extrapolate')
            for j, c in enumerate(ce_cols):
                if pd.isna(row[c]):
                    if ce_s[j] >= v_s[0] and ce_s[j] <= v_s[-1]:
                        filled.at[ix, c] = float(f_in(ce_s[j]))
                    else:
                        filled.at[ix, c] = float(f_ex(ce_s[j]))
        elif len(v_s) >= 2:
            f_ex = interp1d(v_s, v_iv, kind='linear', fill_value='extrapolate')
            for j, c in enumerate(ce_cols):
                if pd.isna(row[c]):
        ...
        if len(v_s) >= 4:
            ...
            for j, c in enumerate(pe_cols):
                if pd.isna(row[c]):
                    if pe_s[j] >= v_s[0] and pe_s[j] <= v_s[-1]:
                        filled.at[ix, c] = float(f_in(pe_s[j]))
                    else:
                        filled.at[ix, c] = float(f_ex(pe_s[j]))
        elif len(v_s) >= 2:
            f_ex = interp1d(v_s, v_iv, kind='linear', fill_value='extrapolate')
            for j, c in enumerate(pe_cols):
                if pd.isna(row[c]):
...
print("Cubic Interior + Linear Extrapolate MSE:", eval_mixed_interp(masked, truth, 'cubic'))
print("Quadratic Interior + Linear Extrapolate MSE:", eval_mixed_interp(masked, truth, 'quadratic'))
print("PCHIP Interior + Linear Extrapolate MSE:", eval_mixed_interp(masked, truth, 'pchip'))
