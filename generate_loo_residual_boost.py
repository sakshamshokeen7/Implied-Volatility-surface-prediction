...
print("Running LOO Causal Residual Boosting. This takes ~60 seconds...")
...
for ix in range(len(df)):
    ...
    if len(history_X) >= 200 and ix % RETRAIN_FREQ == 0:
        X_train = np.array(history_X[-20000:])
        y_train = np.array(history_y[-20000:])
    ...
    # Process CE
    # ----------------------------------------------------
    v_s_ce = [strikes_ce[j] for j, c in enumerate(ce_cols) if pd.notna(row[c])]
    v_iv_ce = [row[c] for j, c in enumerate(ce_cols) if pd.notna(row[c])]
    
    if len(v_s_ce) >= 2:
        f_ce = interp1d(v_s_ce, v_iv_ce, kind='linear', fill_value='extrapolate')
        ...
        # Generate LOO residuals for known points
        if len(v_s_ce) >= 3:
            for k in range(len(v_s_ce)):
                loo_s = v_s_ce[:k] + v_s_ce[k+1:]
                loo_iv = v_iv_ce[:k] + v_iv_ce[k+1:]
                if len(loo_s) >= 2:
                    f_loo = interp1d(loo_s, loo_iv, kind='linear', fill_value='extrapolate')
                    pred_loo = float(f_loo(v_s_ce[k]))
                    res = v_iv_ce[k] - pred_loo
                    x_train = [row_feats['time_of_day'], row_feats['dte'], v_s_ce[k]/S, 1, S, row_feats['inv_S'], row_feats['sq_S'], row_feats['log_S']]
                    history_X.append(x_train)
                    history_y.append(res)
    elif len(v_s_ce) == 1:
        for c in ce_cols:
            if pd.isna(row[c]):
                filled_causal.at[ix, c] = v_iv_ce[0]
    ...
    if len(v_s_pe) >= 2:
        f_pe = interp1d(v_s_pe, v_iv_pe, kind='linear', fill_value='extrapolate')
        ...
        # Generate LOO residuals for known points
        if len(v_s_pe) >= 3:
            for k in range(len(v_s_pe)):
                loo_s = v_s_pe[:k] + v_s_pe[k+1:]
                loo_iv = v_iv_pe[:k] + v_iv_pe[k+1:]
                if len(loo_s) >= 2:
                    f_loo = interp1d(loo_s, loo_iv, kind='linear', fill_value='extrapolate')
                    pred_loo = float(f_loo(v_s_pe[k]))
                    res = v_iv_pe[k] - pred_loo
                    x_train = [row_feats['time_of_day'], row_feats['dte'], v_s_pe[k]/S, 0, S, row_feats['inv_S'], row_feats['sq_S'], row_feats['log_S']]
                    history_X.append(x_train)
                    history_y.append(res)
    elif len(v_s_pe) == 1:
        for c in pe_cols:
            if pd.isna(row[c]):
                filled_causal.at[ix, c] = v_iv_pe[0]
...
sub_df.to_csv('submission_loo_residual_boost.csv', index=False)
print(f"Finished in {time.time() - start_time:.2f}s.")
print(f"Saved submission_loo_residual_boost.csv with {len(sub_df)} rows. 100% causal ML.")
